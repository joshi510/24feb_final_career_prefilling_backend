...
const requireStudentOrCounsellorOrAdmin = requireRole(['STUDENT', 'COUNSELLOR', 'ADMIN']);
...
function parseOptionsToArray(optionsString) {
  ...
  const parts = optionsString.split(new RegExp(`${separator}\\s*(?=[A-E][\\)\\.])`));
  ...
  if (result.length === 0) {
    const pattern = new RegExp(`([A-E])[\\)\\.]\\s*([^${separator}]+?)(?=\\s*[A-E][\\)\\.]|$)`, 'gi');
    let match;
...
router.get('/interpretation/:test_attempt_id', getCurrentUser, requireStudentOrCounsellorOrAdmin, async (req, res) => {
  try {
    const testAttemptId = parseInt(req.params.test_attempt_id, 10);
    const currentUser = req.user;
    ...
    // If student, verify it's their attempt (admin and counsellor can access any)
    ...
    // Check if interpretation already exists and is complete (cached)
    let interpretedResult = await InterpretedResult.findOne({
      where: { test_attempt_id: testAttemptId }
    });
    
    // If cached interpretation exists with all required fields, return it immediately
    if (interpretedResult && 
        interpretedResult.interpretation_text && 
        interpretedResult.readiness_status && 
        interpretedResult.risk_level &&
        interpretedResult.career_direction) {
      console.log(`✅ Using cached interpretation from database for attempt ${testAttemptId}`);
      ...
      try {
        ...
        console.warn(`⚠️ Error parsing cached interpretation fields: ${parseError.message}`);
      }
      
      // Get score data for response
      ...
      const percentage = score ? score.score_value : 0;
      const totalQuestions = score ? Math.round((score.score_value / 100) * 35) : 0;
      const correctAnswers = score ? Math.floor((score.score_value / 100) * 35) : 0;
      
      // Get section scores
      const sectionScores = await Score.findAll({
        where: {
          test_attempt_id: testAttemptId,
          dimension: {
            [Op.like]: 'section_%'
      ...
      const sectionScoresArray = sectionScores.map(s => ({
        section: s.dimension,
        score: s.score_value
      }));
      
      // Get student contact number
      let studentContactNumber = null;
      try {
        const student = await Student.findOne({
          where: { user_id: testAttempt.student_id }
        });
        if (student) {
          studentContactNumber = student.contact_number || student.mobile_number || null;
          console.log(`✅ Found student contact number for user ${testAttempt.student_id}: ${studentContactNumber}`);
        } else {
          console.warn(`⚠️ Student not found for user_id: ${testAttempt.student_id}`);
        }
      } catch (err) {
        console.error(`❌ Failed to fetch student contact number: ${err.message}`);
        console.error(`❌ Error stack: ${err.stack}`);
      }
      
      return res.json({
        summary: interpretedResult.interpretation_text || '',
        student_contact_number: studentContactNumber,
        strengths: Array.isArray(strengths) ? strengths : [],
        weaknesses: Array.isArray(weaknesses) ? weaknesses : [],
        career_clusters: [interpretedResult.career_direction || 'Multi-domain Exploration'],
        risk_level: interpretedResult.risk_level || 'MEDIUM',
        readiness_status: interpretedResult.readiness_status || 'PARTIALLY READY',
        action_plan: (() => {
          ...
          try {
            if (roadmap.phase1 && roadmap.phase1.title && Array.isArray(roadmap.phase1.actions)) {
              const phase1Actions = roadmap.phase1.actions.slice(0, 2).filter(a => a).join(', ');
              actionPlanItems.push(`${roadmap.phase1.title}${phase1Actions ? ': ' + phase1Actions : ''}`);
            }
            if (roadmap.phase2 && roadmap.phase2.title && Array.isArray(roadmap.phase2.actions)) {
              const phase2Actions = roadmap.phase2.actions.slice(0, 2).filter(a => a).join(', ');
              actionPlanItems.push(`${roadmap.phase2.title}${phase2Actions ? ': ' + phase2Actions : ''}`);
            }
            if (roadmap.phase3 && roadmap.phase3.title && Array.isArray(roadmap.phase3.actions)) {
              const phase3Actions = roadmap.phase3.actions.slice(0, 2).filter(a => a).join(', ');
              actionPlanItems.push(`${roadmap.phase3.title}${phase3Actions ? ': ' + phase3Actions : ''}`);
            }
          } catch (e) {
            console.warn(`⚠️ Error building action plan: ${e.message}`);
          }
          return actionPlanItems.length > 0 ? actionPlanItems : ['Action plan is being generated. Please refresh in a moment.'];
        })(),
        overall_percentage: Math.round(percentage * 100) / 100,
        total_questions: totalQuestions,
        correct_answers: correctAnswers,
        is_ai_generated: interpretedResult.is_ai_generated || false,
        readiness_explanation: interpretedResult.readiness_explanation || '',
        risk_explanation: interpretedResult.risk_explanation || '',
        career_direction: interpretedResult.career_direction || 'Multi-domain Exploration',
        career_direction_reason: interpretedResult.career_direction_reason || '',
        roadmap: roadmap || {},
        section_scores: sectionScoresArray,
        counsellor_summary: interpretedResult.counsellor_summary || '',
        readiness_action_guidance: Array.isArray(readinessActionGuidance) ? readinessActionGuidance : [],
        career_confidence_level: interpretedResult.career_confidence_level || 'MODERATE',
        career_confidence_explanation: interpretedResult.career_confidence_explanation || '',
        do_now_actions: Array.isArray(doNowActions) ? doNowActions : [],
        do_later_actions: Array.isArray(doLaterActions) ? doLaterActions : [],
        risk_explanation_human: interpretedResult.risk_explanation_human || interpretedResult.risk_explanation || ''
      });
    }
...
// GET /test/riasec-report/:test_attempt_id
router.get('/riasec-report/:test_attempt_id', getCurrentUser, requireStudentOrCounsellorOrAdmin, async (req, res) => {
  try {
    const testAttemptId = parseInt(req.params.test_attempt_id, 10);
    const currentUser = req.user;
    ...
    if (testAttempt.status !== TestStatus.COMPLETED) {
      return res.status(400).json({
        detail: 'Test must be completed before generating RIASEC report'
      });
    }

    // Check if RIASEC report is cached in database
    const interpretedResult = await InterpretedResult.findOne({
      where: { test_attempt_id: testAttemptId },
      attributes: ['id', 'riasec_report']
    });
    ...
    console.log(`🔵 Calculating RIASEC scores for attempt ${testAttemptId}`);
    const { R, I, A, S, E, C, error: scoringError } = await calculateRIASECScores(testAttemptId);

    if (scoringError) {
      console.error(`❌ RIASEC scoring error: ${scoringError}`);
      // Check if we have any valid scores despite the error
      const hasAnyScores = R > 0 || I > 0 || A > 0 || S > 0 || E > 0 || C > 0;
      if (!hasAnyScores) {
        return res.status(400).json({
          detail: scoringError,
          scores: { R, I, A, S, E, C }
        ...
        console.log(`⚠️ Partial RIASEC scores available despite error: R=${R}, I=${I}, A=${A}, S=${S}, E=${E}, C=${C}`);
        // Continue with partial scores
      }
    }

    // Generate RIASEC report using Gemini
    ...
    if (reportError) {
      console.error(`❌ RIASEC report generation error: ${reportError}`);
      return res.status(500).json({
        detail: reportError
      });
    }
    ...
    if (!report) {
      return res.status(500).json({
        detail: 'Failed to generate RIASEC report'
      });
    }

    console.log(`✅ RIASEC report generated successfully for attempt ${testAttemptId}`);
    return res.json({
      test_attempt_id: testAttemptId,
      scores: { R, I, A, S, E, C },
      report: report
    ...
    console.error(`❌ Unexpected error in RIASEC report endpoint for attempt ${testAttemptId}:`, error);
    console.error(`❌ Error in RIASEC report generation: ${error.message}`);
    console.error(`❌ Error stack: ${error.stack}`);
    return res.status(500).json({
      detail: 'Failed to generate RIASEC report',
...
router.get('/sections', getCurrentUser, requireStudent, async (req, res) => {
  try {
    ...
    // Define all 10 sections - ALWAYS return all 10 sections
    ...
    // Build all_sections list - always 10 sections
    const allSections = [];
...
router.get('/sections/:section_id/questions', getCurrentUser, requireStudent, async (req, res) => {
  try {
    ...
    // SELECT * FROM sections WHERE id = ? OR (order_index = ? AND order_index BETWEEN 1 AND 10)
    let section = await Section.findByPk(sectionId);
    if (!section && sectionId >= 1 && sectionId <= 10) {
