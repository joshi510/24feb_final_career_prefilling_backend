...
function determineCareerDirection(sectionScores, sections, overallPercentage = 0.0) {
  ...
  } else if ([4, 5, 6, 7, 8, 9, 10].includes(maxSectionNum) && (!secondMax || [4, 5, 6, 7, 8, 9, 10].includes(secondMax[0]))) {
    return [
      'Creative / Design / Career Interest',
...
async function generateAndSaveInterpretation(testAttemptId, totalQuestions, correctAnswers, percentage) {
  ...
  const riskExplanationHuman = generateHumanRiskExplanation(riskLevel, readinessStatus);
    ...
    // Save all interpretation data to database for caching
    ...
    if (!interpretedResult) {
      ...
      interpretedResult = await InterpretedResult.create({
        ...
        readiness_status: finalInterpretationData.readiness_status || readinessStatus,
        readiness_explanation: finalInterpretationData.readiness_explanation || readinessExplanation,
        risk_level: finalInterpretationData.risk_level || riskLevel,
        risk_explanation: finalInterpretationData.risk_explanation || riskExplanation,
        career_direction: finalInterpretationData.career_direction || careerDirection,
        career_direction_reason: finalInterpretationData.career_direction_reason || careerDirectionReason,
        roadmap: JSON.stringify(finalInterpretationData.roadmap || roadmap),
        counsellor_summary: counsellorSummary,
